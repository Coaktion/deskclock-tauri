...
- **Ações por tarefa:** Array de `{ type: "open_url" | "open_file", value: string, label?: string }`. URL auto-completa `https://` se ausente. N ações por tarefa. As ações não são disparadas automaticamente ao iniciar. Elas se alcançam **sem iniciar a tarefa e sem abrir a edição**, pelo ⚡ da própria linha (aqui e na lista do popup), e continuam disponíveis como chips durante a execução — no card do popup, no omnibox e no `RunningTaskEditSheet` (ver §6.5). **No modo de seleção o ⚡ some**, pelo mesmo motivo que os seis botões: ali a linha inteira é alvo de marcar, e um controle que engole o clique faria a tarefa recusar a seleção justamente enquanto se escolhe o que excluir em lote.
...
- **Botões por tarefa:** Play | Compartilhar | Editar | Concluir/Pendente | Duplicar | Excluir (sem confirmação). Não há botão "Ações": editar ações é dentro do modal de edição. **Só aparecem no hover da linha, e sem hover não ocupam largura** (`w-0` + `overflow-hidden`, não apenas `opacity-0`): reservados, o espaço de seis botões saía do nome da tarefa, que truncava numa linha vazia à direita. `focus-within` abre o bloco para quem navega pelo teclado. O Play é o único que some — na tarefa já concluída no dia —, então a fileira tem cinco ou seis conforme a linha.
- **Compartilhar copia um link, não abre nada.** O `Share2` monta o `deskclock://task/share` da tarefa (`plannedTaskToSharePayload` + `buildShareLink`), escreve na área de transferência e avisa por toast — é uma ação de um clique só, e um modal para mostrar o endereço só acrescentaria o clique de fechar. O que o link leva e o que ele deliberadamente deixa para trás (agendamento, recorrência, data, ações, ids) está em `docs-internal/specs/compartilhar-tarefa.md`; a linha da planejada é a **única** origem de link no app, e a chegada não é aqui — é o modal que o deeplink abre, de qualquer tela.

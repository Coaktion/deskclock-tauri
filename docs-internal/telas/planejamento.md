...
- **O chip de faturamento fica parado.** A célula do ⋯ abre em largura **antes** do chip, e o Play fica depois dele, numa coluna reservada mesmo na tarefa concluída. A largura que a célula abre sai do `1fr` do nome, e com o chip antes dela ele andava no instante em que o cursor entrava na linha. Desde que o chip virou controle (`bbb4a1b`), posição estável é requisito, não acabamento. Em repouso a célula fechada cancela o `gap` que não ocupa (`-mr-2.5`).

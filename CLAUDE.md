...
- **Primitivos canônicos:** `src/presentation/components/ui/` — `Button`, `IconButton`,
  ...
  > numa **linha** que já espaçou — a busca do acesso rápido, a linha "adicionar" das listas, o
  > editor de renomear. `bare` e `plain` não são o mesmo: o `bare` está numa caixa que traça a borda
  ...
  > fechar — é a janela da primeira execução, não um diálogo. **O `CommandPalette` não é modal**:
  > abre encostado no alto (`items-start pt-20`), não centrado, e não tem cabeçalho, rodapé nem
  > título. Ele é o último `backdrop-blur` do app, e a casca não o expressa.
  >

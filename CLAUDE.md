...
│   ├── overlays/     # CompactOverlay, PopupFlyout, Toast
...
- **Todo modal fecha no ESC**, e quem usa a casca `Modal` (§8.4) **ganha isso de graça** — ela chama
  o `useEscapeToClose` por dentro, junto com o `data-modal-open` que impede o mesmo ESC de esconder
  a janela do app. Fora da casca (os overlays, as gavetas do popup), é `useEscapeToClose(onClose)`
  **mais** o `data-modal-open` no elemento de topo — um único hook, nunca um `addEventListener`
  copiado. Um modal que não fecha no ESC é o único da tela que não fecha, e o usuário só descobre
  qual é tentando. O hook ignora o ESC já consumido (`defaultPrevented`), que é como o
  `Autocomplete` fecha o dropdown sem derrubar o modal junto. Exceção: o `SetupModal`, que não tem
  para onde fechar.

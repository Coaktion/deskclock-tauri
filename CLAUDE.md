...
- **Primitivos canônicos:** `src/presentation/components/ui/` — `Button`, `IconButton`,
  ...
  `FilterPill`, `SearchInput`, `Field`, `Modal`, `PageHeader` e `SectionCard` (com `SectionRow`).
  ...
  >
  > **Sucesso e aviso continuam sem token, e é a maior dívida da paleta.** Nas Integrações, "conectado"

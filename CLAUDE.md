...
- **Primitivos canônicos:** `src/presentation/components/ui/` — `Button`, `IconButton`,
  ...
  `FilterPill`, `Badge`, `SearchInput`, `Field`, `Modal`, `PageHeader` e `SectionCard` (com
  `SectionRow`).
  ...
  > entregues. A linha também carrega chip de billable, ícone de recorrência e sino, que o primitivo
  > não expressa.
  >

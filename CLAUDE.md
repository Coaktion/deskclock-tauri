...
- **Primitivos canônicos:** `src/presentation/components/ui/` — `Toggle`, `KpiCard`, `TaskRow`,
  ...
  `components/SearchInput.tsx` e o `ToggleRow`/`SettingsCard`/`CardRow` do `SettingsShared`
  saíram.

  > **A janela principal abre em 1000×700**, e não mais em 800×620. Foi o que fez as abas caberem
  > no cabeçalho de 56 px: descontados a sidebar (68 px) e o rail (52 px), 800 px deixavam 648 px
  > úteis e as seis abas de Configurações mais o título e o botão Manual pediam ~690. A medida
  > vive em dois lugares que **precisam concordar** — `main` em `tauri.conf.json` e
  > `MAIN_WINDOW_SIZE` em `useStartupWindow`, de onde sai o canto calculado por
  > `positionNearTaskbar`. Divergindo, a janela nasce fora da área útil.

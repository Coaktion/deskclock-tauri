...
# Cópia local, com um bloco anexado ao final, de
# https://github.com/tauri-apps/linuxdeploy-plugin-gtk (master, script único).
# O `scripts/appimage/seed-linuxdeploy-cache.sh` grava esta versão no cache
# onde o bundler do Tauri procura o plugin antes de baixar o dele — então
# essa cópia substitui a original em vez de rodar ao lado dela; qualquer
# atualização da nossa cópia acompanha o upstream linha a linha, exceto pelo
# bloco final.
#
# Por quê: esse plugin embute glib/gobject/gio/gtk vindas do Ubuntu 22.04 (via
# linuxdeploy --library=...) dentro do AppImage. Em qualquer sistema com glib
# >= 2.80 (Arch, Fedora recente, Ubuntu 24.04+), a libayatana-appindicator do
# sistema — carregada por dlopen em runtime, nunca empacotada — é linkada
# contra essa glib nova e referencia símbolos (g_once_init_leave_pointer)
# ausentes na 2.72 embutida. O AppRun resolve a glib para a embutida antes da
# do sistema, o dlopen falha e o app sai em panic antes de abrir janela
# (libappindicator-sys/src/lib.rs:41). As libwayland-* de 2022 embutidas têm o
# mesmo problema com o Mesa/EGL do sistema — janela do webview não renderiza
# (EGL_BAD_PARAMETER). As duas famílias de libs estão na excludelist oficial
# do AppImage por causarem exatamente esse tipo de conflito; retrocompatíveis
# como são, remover as cópias embutidas e deixar o runtime resolver para as do
# sistema não quebra nenhuma distro de 2022 pra cá — foi assim que se validou
# a correção, manualmente, num Manjaro com Hyprland/Wayland.
...
# ── DeskClock: remover libs de baixo nível que conflitam em runtime ────────
# Ver comentário no topo deste arquivo. Cada uma tem retrocompatibilidade
# garantida — a versão do sistema (>= 2022 em qualquer distro) resolve no
# lugar da embutida assim que ela deixa de existir no AppDir.
echo "Removendo libs de baixo nível conflitantes (Arch/Fedora/Ubuntu 24.04+, ver script)"
DESKCLOCK_EXCLUDED_LIBS=(
    "libglib-2.0.so*"
    "libgobject-2.0.so*"
    "libgio-2.0.so*"
    "libgmodule-2.0.so*"
...
FIND_EXPR=()
for pattern in "${DESKCLOCK_EXCLUDED_LIBS[@]}"; do
    [ ${#FIND_EXPR[@]} -gt 0 ] && FIND_EXPR+=( -o )
    FIND_EXPR+=( -name "$pattern" )
done
find "$APPDIR" \( -type f -o -type l \) \( "${FIND_EXPR[@]}" \) -print -delete

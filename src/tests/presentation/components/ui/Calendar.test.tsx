...
describe("Calendar", () => {
  it("abre no mês do valor, não no mês corrente", () => {
    render(<Calendar weekStartsOn={1} value="2026-03-14" onSelect={() => {}} />);
    expect(screen.getByRole("button", { name: "Março" })).toBeTruthy();
    expect(screen.getByRole("button", { name: rotulo("2026-03-14") })).toBeTruthy();
  });

  it("com a semana no domingo, o cabeçalho e a primeira célula andam uma coluna", () => {
    // Setembro de 2026 começa numa terça: segunda-primeiro abre em 31/08,
    // domingo-primeiro em 30/08.
    const { unmount } = render(
      <Calendar weekStartsOn={1} value="2026-09-14" onSelect={() => {}} />
    );
    expect(screen.getAllByRole("button", { name: rotulo("2026-08-31") })).toHaveLength(1);
    expect(screen.queryByRole("button", { name: rotulo("2026-08-30") })).toBeNull();
    unmount();

    render(<Calendar value="2026-09-14" onSelect={() => {}} weekStartsOn={0} />);
    expect(screen.getAllByRole("button", { name: rotulo("2026-08-30") })).toHaveLength(1);
  });

  it("sem valor, abre no mês de hoje", () => {
    render(<Calendar weekStartsOn={1} value="" onSelect={() => {}} />);
  ...
  it("devolve o ISO do dia clicado", () => {
    ...
    render(<Calendar value="2026-03-14" onSelect={onSelect} weekStartsOn={1} />);
  ...
  it("navega de mês pelo ‹ ›, virando o ano em dezembro", () => {
    render(<Calendar weekStartsOn={1} value="2026-12-10" onSelect={() => {}} />);
  ...
  it("desabilita o que passa do `maxISO`", () => {
    render(
      <Calendar weekStartsOn={1} value="2026-03-14" onSelect={() => {}} maxISO="2026-03-20" />
    );
    expect(
  ...
  it("move o foco por seta e atravessa a borda do mês", () => {
    ...
    render(<Calendar weekStartsOn={1} value="2026-03-01" onSelect={() => {}} />);
  ...
  it("a seta não leva o foco além do `maxISO`", () => {
    render(
      <Calendar weekStartsOn={1} value="2026-03-20" onSelect={() => {}} maxISO="2026-03-20" />
    );
  ...
  it("sem rodapé não desenha a régua que o separa da grade", () => {
    const { container, rerender } = render(
      <Calendar weekStartsOn={1} value="" onSelect={() => {}} />
    );
    expect(container.querySelector(".border-t")).toBeNull();

    rerender(
      <Calendar weekStartsOn={1} value="" onSelect={() => {}} footer={<button>Hoje</button>} />
    );

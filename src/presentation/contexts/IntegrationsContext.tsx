...
export function IntegrationsProvider({
  children,
  value,
}: { children: ReactNode; value?: Partial<IntegrationFactories> }) {
  ...
  const defaults = useMemo<IntegrationFactories>(
    () => ({
      ...
      createClockifyApi: (apiKey) =>
        new ClockifyClient(apiKey ?? config.get("clockifyApiKey")),
    }),
    [config]
  );
  const factories = useMemo(() => ({ ...defaults, ...value }), [defaults, value]);
  return (
    <IntegrationsContext.Provider value={factories}>
      {children}

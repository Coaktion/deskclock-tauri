...
/**
 * Marca do Google Agenda no desenho de 2026 (o dos gradientes), copiada do SVG
 * oficial servido pelo Google em
 * `www.gstatic.com/images/branding/productlogos/calendar_2026/v2/web/192px.svg`.
 *
 * O `viewBox` recorta a folga do original (192 × 192) em volta da arte, numa
 * caixa ainda quadrada — como a dos outros logos de integração —, para o ícone
 * ocupar a placa do rail e o cabeçalho das sub-seções no mesmo tamanho deles.
 */
export function GoogleCalendarLogo({ size = 20 }: { size?: number }) {
  ...
  return (
    ...
    >
      ...
      />
      <path fill="#3c90ff" d={CALENDAR_BODY_PATH} />
      <mask
        id={`${id}-mb`}
        width="154"
        height="152"
      ...
      <g mask={`url(#${id}-mb)`}>
        <path fill={`url(#${id}-b)`} d="M0 0h166v76H0z" transform="matrix(1 0 0 -1 13 172)" />
      </g>
      <mask
        id={`${id}-mt`}
        width="154"
        ...
        style={{ maskType: "alpha" }}
      >
        <path fill="#3186ff" d={CALENDAR_BODY_PATH} />
      </mask>
      <g mask={`url(#${id}-mt)`}>
        <path
          fill={`url(#${id}-t)`}
      ...
      />
      <defs>
        <linearGradient id={`${id}-b`} x1="83" x2="83" y1="76" gradientUnits="userSpaceOnUse">

...
function quickToRange(quick: QuickPeriod, customStart: string, customEnd: string): PeriodRange {
  ...
  switch (quick) {
    ...
    case "week": return { start: addDaysISO(today, -6), end: today };
...
export function ClockifySendModal({ projects, categories, onClose }: ClockifySendModalProps) {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (!cancelled) {
          const msg = err instanceof Error ? err.message : typeof err === "string" ? err : "Erro ao carregar tarefas.";
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (err instanceof NoIntegrationError) {
        setMessage({ text: "Clockify não configurado. Verifique API Key e workspace.", error: true });
  ...
  const QUICK_LABELS: Record<QuickPeriod, string> = {
    today: "Hoje", yesterday: "Ontem", week: "7 dias", month: "Mês", custom: "Período",
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          <div>
            <h2 className="text-sm font-semibold text-gray-100">Enviar para Clockify</h2>
            ...
            {(Object.keys(QUICK_LABELS) as QuickPeriod[]).map((q) => (
              <button
                ...
                className={`px-3 py-1 text-xs rounded-full transition-colors ${
                  quick === q ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:text-gray-200"
                }`}
              >
                {QUICK_LABELS[q]}
              </button>
                ...
                return (
                  <div key={date} className="rounded-lg overflow-hidden">
                    ...
                    >
                      ...
                      <span className="text-[11px] text-gray-500">{selectedCount}/{groups.length}</span>
        ...
        {hasSentSelected && (
          <div className="mx-4 mb-2 flex items-start gap-2 px-3 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            ...
            <p className="text-xs text-yellow-300">
              Uma ou mais tarefas selecionadas já foram enviadas. O reenvio pode criar duplicatas no Clockify.
            </p>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-t border-gray-800">
          ...
          >
            {sending ? (
              ...
              <>Enviar {selectedKeys.size > 0 ? `(${selectedKeys.size})` : ""}</>
            )}
          </button>
        </div>
      </div>
